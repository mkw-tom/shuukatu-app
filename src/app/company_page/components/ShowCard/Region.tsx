  ...
  return (
    ...
    >
      <ul className="flex flex-col gap-1">
        <li
          className={`border-l-2 ${selectPost?.failed ? 'border-l-error' : 'border-l-info'} ${selectPost?.completed ? 'border-l-orange-500' : ''} pl-2 text-gray-900 dark:text-gray-400`}
        >
          場所： {selectPost?.region}
        </li>
        <li
          className={`flex items-start border-l-2 ${selectPost?.failed ? 'border-l-error' : 'border-l-info'} ${selectPost?.completed ? 'border-l-orange-500' : ''} pl-2 text-gray-900 dark:text-gray-400`}
        >
